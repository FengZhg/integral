...
func (r *RedisHandler) Query(ctx *gin.Context, req *logic.QueryReq, rsp *logic.QueryRsp) error {
	var queryRsps []*logic.SingleQueryRsp
	for _, uid := range req.GetUids() {
		...
		queryRsps = append(queryRsps, &logic.SingleQueryRsp{
			Uid:      uid,
			Appid:    req.GetAppid(),
			Type:     req.GetType(),
			Integral: balance,
